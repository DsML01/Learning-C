...
btree *create_tree(int item, btree *left, btree *right, btree *pai)
{
...
btree *construct_btree(char tree[], int *i, btree *pai)
{
    if(tree[*i] == '(' && tree[*i + 1] == ')')
    {
        *i += 2;
    ...
    btree *reggae_roots = create_tree(finally_numbers, NULL, NULL, pai);

    reggae_roots->left = construct_btree(tree, i, reggae_roots);
    reggae_roots->right = construct_btree(tree,i, reggae_roots);
...
int main()
{
    ...
    int i = 0;
    btree *root = construct_btree(matriz, &i, NULL);

    //if(root->pai == NULL) puts("EH NULOOO");
    ...
    while(1)
    {
        ...
        int i2 = 0;
        btree *aux_root = construct_btree(nice_tree, &i2, NULL);
        int procurado = aux_root->item;

        if(procurado != root->item)
        {
            btree *pecheur = NULL;
            ...
            if(pecheur->pai->left == pecheur)
            {
                painovo = pecheur->pai->left;
                pecheur->pai->left = aux_root;
                aux_root->pai = painovo;
            }
            else if (pecheur->pai->right == pecheur)
            {
                painovo = pecheur->pai->right;
                pecheur->pai->right = aux_root;
                aux_root->pai = painovo;
            }
            else if(pecheur == NULL)
            {
                root = aux_root;
                printarvore(root);
            }
