...
/*bool haDuplicados(ARVORE_BINARIA *arvore, ARVORE_BINARIA *raiz)
{
   ...
   // Verifica duplicatas na subárvore à esquerda e à direita
   return haDuplicados(arvore->esq, raiz) || haDuplicados(arvore->dir, raiz) || (arvore != raiz && arvore->item.chave == raiz->item.chave);
   // Verifica duplicatas em relação à raiz 
}*/
...
void imprimirSeHaDuplicados(ARVORE_BINARIA *arv) {
   if (haDuplicados(arv, arv)) printf("Ha valores duplicados\n");

   else printf("Nao ha valores duplicados\n");
}
