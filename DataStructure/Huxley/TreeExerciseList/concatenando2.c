...
btree *jatem(btree *arvore, int wanted, btree **pecheur, btree *pecheurdepere)
{
    *pecheur = arvore;

    //if(arvore == NULL) return pecheurdepere;

    if(arvore->item == wanted) return pecheurdepere;

    //*pecheurdepere = arvore;
    jatem(arvore->left, wanted, pecheur, arvore);

    jatem(arvore->right, wanted, pecheur, arvore);
   
   return NULL;
}

/*btree *receba_a_inteligencia(btree *arvore, int wanted, btree **pecheur, btree **pecheurdepere)
{
    return jatem(arvore, wanted, pecheur, pecheurdepere) == 1 ? arvore : NULL; 
}*/
...
int main()
{
    ...
    while(1)
    {
        ...
        //btree *pecheurdepere = NULL;
        btree *pecheur = NULL;
        btree *pecheurdepere = jatem(root, procurado, &pecheur, NULL);

        printf("%d\n\n\n\n", pecheurdepere->item);

        //printf("%d\n\n\n\n", pecheur->item);

        if(pecheurdepere != NULL)
        {
            if(pecheurdepere->left == pecheur)
            {
                pecheurdepere->left = aux_root;
            }

            if(pecheurdepere->right == pecheur)
            {
                pecheurdepere->right = aux_root;
            }
        }
